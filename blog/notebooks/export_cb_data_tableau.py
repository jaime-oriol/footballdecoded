...
def extract_transfermarkt(df):
    """Extract Transfermarkt data (categorical/mixed types)."""
    tm_keys = [
        'transfermarkt_club', 'transfermarkt_birth_date', 'transfermarkt_primary_foot',
        'transfermarkt_market_value_eur', 'transfermarkt_contract_end_date',
        'transfermarkt_position_specific', 'transfermarkt_contract_is_current',
        'transfermarkt_contract_start_date', 'transfermarkt_transfermarkt_player_id'
    ]
    result = pd.DataFrame(index=df.index)
    for key in tm_keys:
        values = []
        for _, row in df.iterrows():
            if isinstance(row['transfermarkt_metrics'], dict) and key in row['transfermarkt_metrics']:
                values.append(row['transfermarkt_metrics'][key])
            else:
                values.append(None)
        result[key] = values
    return result
...
def main():
    ...
    SELECT unique_player_id, player_name, team, league, season, position, nationality, age,
           fbref_metrics, transfermarkt_metrics
    ...
    # Filter CBs: Transfermarkt = 'CB' AND FBref contains 'DF'
    tm_is_cb = df_raw['transfermarkt_metrics'].apply(
        lambda x: x.get('transfermarkt_position_specific', '') if isinstance(x, dict) else ''
    ) == 'CB'
    fbref_is_df = df_raw['position'].str.contains('DF', case=False, na=False)
    df_cb = df_raw[tm_is_cb & fbref_is_df].copy()
    print(f"CBs (TM=CB + FBref=DF): {len(df_cb)} players")

    # Extract metrics
    fbref_nums = extract_metrics(df_cb, 'fbref_metrics')
    transfermarkt_data = extract_transfermarkt(df_cb)
    print(f"Metrics: FBref {fbref_nums.shape[1]}, Transfermarkt {transfermarkt_data.shape[1]}")

    # Per 90 normalization (FBref only)
    ...
    # Per 100 touches normalization (FBref only)
    ...
    transfermarkt_data = transfermarkt_data.reset_index(drop=True)
