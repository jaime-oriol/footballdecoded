...
BACKGROUND_COLOR = '#313332'  # Color de fondo principal
PITCH_COLOR = '#313332'       # Color del campo (mismo que fondo)

def calculate_node_size(total_passes: int, max_passes: int, threshold: int = 20) -> float:
    """
    Calcula el tamaño del nodo basado en el número de pases completados.
    
    MODIFICAR AQUÍ para cambiar tamaños de nodos:
    - min_size: tamaño mínimo de nodo (jugadores con pocos pases)
    - max_size: tamaño máximo de nodo (jugadores con muchos pases)
    - Escalado lineal entre 5 y 100 pases
    """
    min_size = 5   # Tamaño mínimo
    max_size = 30  # Tamaño máximo
    ...
    # Escalado lineal entre min y max
...
def calculate_line_width(pass_count: int, min_connections: int, max_connections: int, min_required: int = 5) -> float:
    """
    Calcula el grosor de las líneas de conexión basado en número de pases.
    
    MODIFICAR AQUÍ para cambiar grosores de líneas:
    - min_required: mínimo de pases para mostrar conexión
    - Escalado por rangos: 5-7 (fino), 7-12 (medio), 12+ (grueso)
    """
    if pass_count < min_required:
        return 0.0  # No mostrar conexión
    
    if pass_count <= 7:
        return 0.5   # Línea fina
    elif pass_count <= 12:
        return 1.5   # Línea media
    else:
        return 2.5   # Línea gruesa

def get_node_radius(marker_size: float) -> float:
    """
    Convierte el tamaño del marcador a radio real para cálculos geométricos.
    Factor 0.32 ajusta la conversión visual área->radio.
    """
...
def draw_connection_arrow(ax, start_x: float, start_y: float, end_x: float, end_y: float,
    ...
    """
    Dibuja la punta de flecha al final de cada conexión.
    
    PARÁMETROS VISUALES MODIFICABLES:
    - size: tamaño base de la flecha
    - extension: extensión de la punta
    - linewidth multiplier: grosor relativo a la línea
    """
    ...
    size = max(0.6, line_width * 0.25)  # MODIFICAR: tamaño de flecha
    extension = size * 0.05             # MODIFICAR: extensión de punta
...
def optimize_name(full_name: str) -> str:
    """
    Optimiza nombres de jugadores para display (Inicial + Apellido).
    Maneja partículas como 'de', 'del', 'van', etc.
    
    MODIFICAR AQUÍ para cambiar formato de nombres mostrados.
    """
...
def plot_pass_network(network_csv_path, info_csv_path, aggregates_csv_path,
    ...
    """
    FUNCIÓN PRINCIPAL: Genera visualización de red de pases con dos equipos lado a lado.
    
    ESTRUCTURA DE LA VISUALIZACIÓN:
    1. Carga y procesa datos de 3 CSVs
    2. Configura matplotlib con 2 subplots (un equipo por lado)
    3. Para cada equipo: dibuja campo, conexiones, nodos, nombres
    4. Añade leyendas, logos, títulos y metadata
    """
    ...
    # Convertir coordenadas Opta a coordenadas de campo vertical
    positions_df['x_pitch'] = positions_df['avg_y_start']  # Y original -> X campo
    positions_df['y_pitch'] = positions_df['avg_x_start']  # X original -> Y campo
    
    # === CONFIGURACIÓN DE RANGOS PARA COLORES ===
    # MODIFICAR AQUÍ para cambiar rangos de colores
    min_connection_xt = -0.1  # xThreat mínimo para conexiones
    max_connection_xt = 0.2   # xThreat máximo para conexiones
    
    min_player_xt = 0.0       # xThreat mínimo para jugadores
    max_player_xt = 0.05      # xThreat máximo para jugadores
    
    min_passes = 6            # Mínimo de pases para mostrar conexión

    def change_range(value, old_range, new_range):
        """Función auxiliar para cambiar rangos (no usada actualmente)"""
    ...
    # === CONFIGURACIÓN DE MAPAS DE COLOR ===
    # Normalizadores para mapear valores a colores
    ...
    # MODIFICAR AQUÍ para cambiar colores de la escala
    node_cmap = mcolors.LinearSegmentedColormap.from_list("", [
        'deepskyblue',  # Azul para valores bajos
        'cyan',         # Cian
        'lawngreen',    # Verde claro
        'yellow',       # Amarillo
        'gold',         # Dorado
        'lightpink',    # Rosa claro
        'tomato'        # Rojo para valores altos
    ])
    
    # === LOOP PRINCIPAL: PROCESAMIENTO POR EQUIPO ===
    for i, team in enumerate(teams):
        ...
        # === DIBUJAR CONEXIONES ENTRE JUGADORES ===
        ...
        if not valid_connections.empty:
            ...
            for _, conn in valid_connections.iterrows():
                ...
                # Calcular puntos de conexión evitando solapamientos
                ...
                # Gradiente de transparencia: más transparente al inicio, opaco al final
        ...
        # === DIBUJAR NODOS DE JUGADORES (ESTRUCTURA ORIGINAL + GROSOR REDUCIDO) ===
    ...
    # === LOGOS DE EQUIPOS ===
    # MODIFICAR AQUÍ las posiciones de los logos
    ...
    # === DIBUJAR ELEMENTOS DE LEYENDA ===
    # COORDENADAS PARA ELEMENTOS DE LEYENDA (sistema de coordenadas de figura)
    x0 = 225    # X base para flechas de grosor
    y0 = 250    # Y base
    dx = 60     # Ancho de flecha
    dy = 120    # Alto de flecha
    shift_x = 70  # Separación horizontal
    
    x1 = 740    # X base para flechas de color
    x2 = 1400   # X base para círculos de tamaño
    y2 = 310    # Y para círculos
    shift_x2 = 70  # Separación círculos
    radius = 20    # Radio base círculos
    
    x3 = 1840   # X base para círculos de color
    shift_x3 = 100  # Separación círculos de color
    
    x4 = 960    # X base para flecha de escala
    y4 = 100    # Y para flecha de escala
    
    # Estilo de flecha
    style = ArrowStyle('->', head_length=5, head_width=3)
    
    # === LEYENDA 1: GROSOR DE LÍNEAS ===
    # MODIFICAR AQUÍ para cambiar grosor de flechas en leyenda
    ...
    # === LEYENDA 2: COLORES DE CONEXIONES ===
    ...
    # === LEYENDA 3: TAMAÑOS DE NODOS ===
    # MODIFICAR AQUÍ para cambiar tamaños en leyenda
    ...
    # === LEYENDA 4: COLORES DE NODOS ===
    # ACTUALIZADO: Misma estructura que nodos de jugadores
    for idx, (x_pos, color) in enumerate([
        ...
        # Borde nítido (linewidth=1 como los jugadores ahora)
    ...
    # === FLECHA DE ESCALA LOW-HIGH ===
    arrow9 = FancyArrowPatch((x4, y4), (x4+550, y4), lw=1, arrowstyle=style, color='white')
    
    # Añadir todos los elementos de leyenda a la figura
