...
Specialized goal visualization showing the origin of each goal.
...
def plot_goals_origin(csv_path, player_name, face_path=None, team_name=None,
    ...
    for _, goal in goals_df.iterrows():
        if pd.notna(goal['x']) and pd.notna(goal['y']):
            ...
            scatter = ax['pitch'].scatter(
                goal['y'], goal['x'],  # Usar (y, x) como shot_xg
                s=200,
                c=xg_val,
                cmap=node_cmap,
    ...
    # Column 2 Row 1: Individual Carry
    ...
    # Labels derecha (adapta contenido pero mantiene posición)
